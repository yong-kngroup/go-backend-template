...
var Tracer = otel.Tracer("github.com/freeDog-wy/go-backend-template/internal/infra/kafka")
...
func (c kafkaHeaderCarrier) Set(key, value string) {
	...
	*c.headers = append(*c.headers, kgo.Header{Key: key, Value: []byte(value)})
}
...
func TraceIDFromHeaders(headers []kgo.Header) string {
	if traceID := strings.TrimSpace(HeaderValue(headers, traceIDHeader)); traceID != "" {
		return traceID
	}
...
func InjectTraceContext(ctx context.Context, headers []kgo.Header) []kgo.Header {
	cloned := append([]kgo.Header(nil), headers...)
...
func SerializeHeadersTraceContext(headers []kgo.Header) string {
